...
const Requirements = () => {
    ...
    async function getInquiries() {
      try {
        const response = await axios.get(
          `https://our-lady-of-lourdes-parish-church-tagaytay-city-backend.vercel.app//api/Weddingreq`
        );
  ...
  return (
    <>
      <Breadcrumb pageName="Wedding Client Requirements" />
                  ...
                  <tr key={index}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">
                        {data.brideName.toUpperCase()} {data.brideLastName.toUpperCase()}
                      </p>
                    </td>
                    ...
                    <td className="border-b border-[#eee] py-5 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <button className="hover:text-primary">
                          <Link to={`/weddingAdmin/RequirementsInfo/${data._id}`}>
                            <GoEye size={20} />
                          </Link>
                        </button>
                        <Link to={`/weddingAdmin/editRequirements/${data._id}`}><FaRegEdit size={20} style={{color: '#3C50E0'}}/></Link>
                      </div>
                    </td>
                  </tr>
                ))
