...
import BaptismalEmpty from '../../components/other/BaptismalEmpty';
import { Link } from 'react-router-dom';
import RejectBaptismal from '../../components/modal/RejectBaptismal/RejectBaptismal';
          ...
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              <th className="min-w-[220px] py-4 px-4 font-bold text-black dark:text-white xl:pl-11">
                {(`Name to be baptized`).toLocaleUpperCase()}
              </th>
              <th className="min-w-[150px] py-4 px-4 font-bold text-black dark:text-white">
                {(`Father Name`).toLocaleUpperCase()}
              </th>
              <th className="min-w-[150px] py-4 px-4 font-bold text-black dark:text-white">
                {(`Mother Name`).toLocaleUpperCase()}
              </th>
              <th className="min-w-[150px] py-4 px-4 font-bold text-black dark:text-white">
                {(`Date of Baptismal`).toLocaleUpperCase()}
              </th>
            ...
            data.map((data, index) => (
              <tr key={index}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">{data.fatherName.toLocaleUpperCase()}</p>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-4.5">
                  <h5 className="font-medium text-black dark:text-white">
                    {data.motherName.toLocaleUpperCase()}
                  </h5>
                </td>
                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-4.5">
                  <h5 className="font-medium text-black dark:text-white">
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <Link to={`/weddingAdmin/BaptismalInfo/${data._id}`}>
                      <GoEye size={20}/>
                    </Link>
                    </button>
                    {/* <button
