...
const AddReport = () => {
  ...
  const [TimeOfWedding, setTimeOfWedding] = useState('08:30'); // Set time in "HH:MM" format
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    axios.post('http://localhost:5000/api/ReportModule/create', reportData)
      .then(res => {
        console.log(res)
        toast.success('Successfully added report');
        navigate('/weddingAdmin/reports');
      })
      .catch(err => {
        console.log(err);
              ...
              <div className="p-6.5">
                ...
                <div className="mb-4.5">
                  ...
                  <input
                      ...
                      className="w-full rounded border border-stroke bg-gray py-3 px-4.5 text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
                      value={TimeOfWedding}
                      onChange={(e) => setTimeOfWedding(e.target.value)}
                    />
                  </div>
                </div>
                <div className="mb-4.5">
