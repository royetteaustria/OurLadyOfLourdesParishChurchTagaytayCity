    ...
    <div className="w-full sm:px-6 ">
      <div className="px-4 md:px-10 py-4 md:py-7 text-black dark:text-white bg-white rounded-tl-lg rounded-tr-lg dark:bg-boxdark" >
          ...
          <div className={data.length > 0 ? 'inline-flex sm:ml-3 mt-4 sm:mt-0 items-start justify-start px-6 py-3 bg-primary focus:outline-none rounded' : 'hidden'}>
            <button >
              <Link to='/ParishSecretary/Addrecord'><p className="text-sm font-medium leading-none text-white">Add record</p></Link>
            </button>
            </div>
          </div>
        </div>
          ...
          <table className="w-full whitespace-nowrap">
            <thead>
              <tr className="h-16 w-full text-sm leading-none text-gray-800 bg-gray-2 dark:bg-meta-4">
                <th className="font-bold text-black dark:text-white text-left pl-24">{(`Name`).toUpperCase()}</th>
                <th className="font-bold text-black dark:text-white text-left pl-12">{(`Date of Baptismal`).toUpperCase()}</th>
                <th className="font-bold text-black dark:text-white text-left ">{(`Actions`).toUpperCase()}</th>
              </tr>
              </thead>
                  ...
                  <tr key={record._id} className="h-20 text-sm leading-none text-gray-800 bg-white hover:bg-gray-100 border-b border-[#eee] dark:border-strokedark dark:bg-boxdark">
                    <td className="pl-4 ">
                      <div className="flex items-center">
                        <div className="pl-20">
                          <p className="font-normal text-black dark:text-white">{record.name.toUpperCase()}</p>
                        </div>
                      </div>
                    </td>
                    <td className="pl-12">
                      <div className="flex items-center">
                        <div className="pl-0">
                          <p className="font-normal text-black dark:text-white">{formatDateTime(record.dateofBaptismal).toUpperCase()}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <button className="hover:text-primary">
                          <Link to={`/ParishSecretary/ViewRecordInfo/${record._id}`}>
