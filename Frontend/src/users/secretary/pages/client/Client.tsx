...
const Client = () => {
    ...
    useEffect(() => {
      async function getInquiries() {
        try {
          const response = await axios.get(`http://localhost:5000/api/MassClient/getAll`);
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              <th className="min-w-[220px] py-4 px-12 font-semibold text-black dark:text-white xl:pl-11">
                {(`Name`).toUpperCase()}
              </th>
              ...
              <th className="min-w-[150px] py-4 px-4 font-semibold pl-12 text-black dark:text-white">
                {(`Status`).toUpperCase()}
              </th>
              ...
              <tr key={index}>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  ...
                  <button className="hover:text-primary">
                      <Link to={`/ParishSecretary/ClientInfo/${data._id}`}>
                          <GoEye size={20}/>
                      </Link>
                    </button>
                    <button className="hover:text-primary">
                      <Link to={`/ParishSecretary/UpdateMassClient/${data._id}`}>
                          {(`Update`).toUpperCase()}
                      </Link>
                    </button>
                  </div>
