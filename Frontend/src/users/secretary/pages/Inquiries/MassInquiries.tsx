...
import EmptyInquiries from "../../components/other/EmptyInquiries"
import toast from 'react-hot-toast';
import { GoEye } from "react-icons/go";
import { MdCancel } from "react-icons/md";
import Breadcrumb from "../../components/breadcrumbs/Breadcrum";
import { Link } from "react-router-dom";
import RejectMass from "../../components/modal/RejectMassInquiry/RejectMass";
...
const MassInquiries = () => {
  ...
  useEffect(() => {
    async function getInquiries() {
      const response = await axios.get(`https://our-lady-of-lourdes-parish-church-tagaytay-city-backend.vercel.app//api/massInquiries/listofInquire`)
            ...
            data.map((data, index) => (
              <tr key={index}>
                ...
                {/* <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <p className="text-black dark:text-white">{formatDateTime(data.date)}</p>
                </td> */}
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    <button className="hover:text-primary">
                      <Link to={`/ParishSecretary/ViewInquiriesInfo/${data._id}`}>
                          <GoEye size={20} />
                      </Link>
                    </button>
