...
import DeleteWeddingDialog from "../../../../../weddingAdmin/components/modal/DeleteRecord/DeleteWeddingRecord";
        ...
        <div className="px-4 md:px-10 py-4 md:py-7 text-black dark:text-white bg-white rounded-tl-lg rounded-tr-lg dark:bg-boxdark">
          <div  className="sm:flex items-center justify-between bg-gray-2 border-b border-[#eee] p-4 dark:bg-meta-4 dark:border-strokedark">
            ...
            <div className={data.length > 0 ? 'inline-flex sm:ml-3 mt-4 sm:mt-0 items-start justify-start px-6 py-3 bg-primary focus:outline-none rounded' : 'hidden'}>
              <button>
                <Link to='/systemAdmin/addWeddingRecords'><p className="text-sm font-medium leading-none text-white">Add record</p></Link>
              </button>
            </div>
          </div>
        </div>
          ...
          <table className="w-full whitespace-nowrap">
            <thead>
              <tr className="h-16 w-full text-sm leading-none text-gray-800 bg-gray-2 dark:bg-meta-4">
                <th className="font-medium text-black dark:text-white text-left pl-4">Groom name</th>
                <th className="font-medium text-black dark:text-white text-left pl-12">Bride name</th>
                <th className="font-medium text-black dark:text-white text-left pl-12">Date of marriage</th>
              ...
              {data.length > 0 ? (
                  ...
                  <tr key={record._id} className="h-20 text-sm leading-none text-gray-800 bg-white hover:bg-gray-100 border-b border-[#eee] dark:border-strokedark dark:bg-boxdark">
                    <td className="pl-4">
                      <div className="flex items-center">
                        <div className="pl-0">
                          <p className="font-normal text-black dark:text-white">{record.groomName}</p>
                        </div>
                      </div>
                    </td>
                    <td className="pl-12">
                      <div className="flex items-center">
                        <div className="pl-0">
                          <p className="font-normal text-black dark:text-white">{record.brideName}</p>
                        </div>
                      </div>
                    </td>
                    <td className="pl-12">
                      <p className="font-normal text-black dark:text-white">{record.dateOfMarriage}</p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        <button className="hover:text-primary">
                          <Link to={`/systemAdmin/Info/${record._id}`}>
                            <AiOutlineEye size={20} />
                          </Link>
                        </button>
                        <button className="text-warning">
                          <Link to={`/systemAdmin/editRecords/${record._id}`}><FaRegEdit size={20} style={{color: '#3C50E0'}}/></Link>
                        </button>
                        <button
                ...
                <tr>
                  <td colSpan={4} className="text-center">
                    <EmptyStates navigate='/systemAdmin/addWeddingRecords' description="" Title="record"/>
                  </td>
                </tr>
              )}
            </tbody>
