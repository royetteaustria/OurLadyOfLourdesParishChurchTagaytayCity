...
const ViewModal: React.FC<ViewModalProps> = ({ event, onClose }) => {
  ...
  // const GoToEditEvent = () => {
  //   navigate(`/systemAdmin/EditEvent/${event._id}`);
  // }

  // async function handleDeleteEvent() {
  //   try {
  //     await axios.delete(`https://our-lady-of-lourdes-parish-church-tagaytay-city-backend.vercel.app//api/Events/delete/${event._id}`);
          ...
          >
            <div data-aos="zoom-in" className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2 pt-2">
                      <p className="text-black text-md py-2 font-semibold"> Title: <span className="font-normal">{event.title}</span></p>
                      ...
                      <p className="text-black text-md py-2 font-semibold"> Details: <span className="font-normal">{event.describe === '' ? 'No details yet' : event.describe}</span></p>
                      <p></p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                ...
                <button
                  type="button"
                  className="w-full inline-flex mb-4 justify-center rounded-md shadow-sm px-4 bg-danger py-2 text-base font-medium text-white hover:bg-red-700 sm:ml-3 sm:w-auto sm:text-sm"
                ...
                <button
                  type="button"
                  className="w-full inline-flex mb-4 justify-center rounded-md border border-primary hover:bg-danger hover:text-white shadow-sm px-4 py-2 text-base font-medium text-black hover:bg-red-700 sm:ml-3 sm:w-auto sm:text-sm"
                  onClick={onClose}
                >
                  Close
                </button>
