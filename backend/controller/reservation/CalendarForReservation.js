  ...
  try {
    ...
    if (existingReservation) {
      return res
        .status(409)
        .json({
          error: "A reservation already exists for the specified time period.",
        });
    }

    const newReservation = await CalendarForReservation.create({
      start,
      end,
      description,
    });
...
const EditReservation = async (req, res) => {
  ...
  try {
    const updateReservation = await CalendarForReservation.findByIdAndUpdate(
      id,
      req.body,
      { new: true }
    );
...
const DeleteReservation = async (req, res) => {
  const id = req.params.id;
  try {
    const Reservationdelete = await CalendarForReservation.findByIdAndDelete(
      id
    );
...
const getAvailableDates = async (req, res) => {
  try {
    const availableDates = await CalendarForReservation.find({
